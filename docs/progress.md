...
**Task 46 — cutover**: the k3s pod is live and healthy at `http://llmgw.arch.local/` with the full 38-model registry. Compose container still listens on host port 51277. To complete cutover:

1. (Optional, recommended) Add `192.168.1.163 llmgw.arch.local` to `/etc/hosts` on the dev box + Mac so the new URL works without `--resolve`.
2. Run `POST http://llmgw.arch.local/v1/models/validate` end-to-end and review the report (will write to the new pod's PVC at `/home/node/.llm-gateway/state.db`).
3. Stop the compose container: `cd ~/agentic/llm-gateway && docker compose -f deploy/compose.yaml down`. Confirm `ss -tlnp | grep 51277` returns nothing.
4. Update any clients still pointing at `http://localhost:51277` → `http://llmgw.arch.local`.
5. Once verified stable for a day or two, delete `deploy/compose.yaml` + `deploy/llmgw` (the CLI wrapper). Keep `deploy/Dockerfile` (still used by GHA).

See [k3s-migration/01-PLAN.md](k3s-migration/01-PLAN.md) §4 for the full cutover acceptance checklist.
...
| 45 | arch-infra registration | ✅ Done | `arch-infra/apps/llmgw.yaml` committed + pushed. ArgoCD synced + healthy; pod `1/1 Running`; `http://llmgw.arch.local/` returns 200; `/v1/models` returns 38 models matching the compose container (9 Ollama / 22 Anthropic / 7 Codex). End-to-end smoke test (Ollama chat completion via Ingress) returned a real response. **Three follow-ups required during sync** (worth knowing if other apps land here later): (1) GHA's `gh repo clone` doesn't bake the PAT into the push URL — switched to `git clone https://x-access-token:${GH_TOKEN}@github.com/...`. (2) The cluster's ArgoCD `argocd-cm` excludes both `v1/Endpoints` and `discovery.k8s.io/EndpointSlice` from sync (UI-clutter reduction default), so the manual Service+Endpoints redirection for Ollama silently dropped — replaced with direct `OLLAMA_BASE_URL: http://192.168.1.163:11434`. (3) `deploy/chart/templates/ollama-service.yaml` removed; `ollama.{hostIp,port}` values entries removed; chart now renders 4 resources (was 6). |
