...
| 46 | Cutover + retire compose | ✅ Done (compose container down; deploy/compose.yaml + deploy/llmgw retained for now) | `/etc/hosts` entry added; compose container `docker compose down` clean; host port 51277 verified free. Full smoke test passed against `http://llmgw.arch.local/`: health, `/v1/models` (38 = 9 Ollama + 22 Anthropic + 7 Codex), Ollama+Anthropic chat (sync + streaming), embeddings (single + batch), vision direct (qwen3-vl:8b → "red") and vision fallback (qwen3:30b via fallback chain → "red"), unknown-model 404, `/docs`. `/v1/models/validate` returned 24 ok / 14 error (Codex+Gemini are upstream-dead from lost subscriptions, expected). SQLite + WAL files confirmed on the PVC at `/home/node/.llm-gateway/`. Pod restart preserves the validation report and the Anthropic OAuth chain (`auth.credentials.loaded source: "db"`, no refresh fired). `deploy/compose.yaml` + `deploy/llmgw` left in tree pending a stability bake; remove in a follow-up commit. |
